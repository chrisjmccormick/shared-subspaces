...
from ethos import EthosConfig, EthosForCausalLM
...
def main():
    ...
    best_checkpoint = full_cfg['pre_train'].get('best_checkpoint')
    if best_checkpoint and os.path.exists(best_checkpoint):
        # Create ETHOS config from model config
        config = EthosConfig(**model_cfg)
        
        # Initialize the ETHOS model and load from checkpoint
        model = EthosForCausalLM.from_pretrained(best_checkpoint, config=config)
    else:
        assert False, "Best checkpoint not found"
